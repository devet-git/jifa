...
import type { IssueType, IssuePriority, IssueStatus, SprintStatus } from "@/types";
...
const priorityColors: Record<IssuePriority, string> = {
  ...
  high:
    "bg-orange-50 text-orange-700 ring-1 ring-orange-200 dark:bg-orange-500/15 dark:text-orange-300 dark:ring-orange-500/20",
  urgent:
...
const typeColors: Record<IssueType, string> = {
  ...
  story:
    "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200 dark:bg-emerald-500/15 dark:text-emerald-300 dark:ring-emerald-500/20",
  epic:
    "bg-violet-50 text-violet-700 ring-1 ring-violet-200 dark:bg-violet-500/15 dark:text-violet-300 dark:ring-violet-500/20",
  subtask:
...
interface BadgeProps {
  type?: "status" | "priority" | "issueType" | "sprint";
  value: string;
  className?: string;
  showDot?: boolean;
}

export function Badge({ type = "status", value, className, showDot }: BadgeProps) {
  let colorClass = "";
  let label = value;
